...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None):
    ...
    rays_o = rays_o.view(-1, H, W, 3)
    rays_d = rays_d.view(-1, H, W, 3)
...
# class SMPLXLiteSeg:
#     smplx_lite_dir = "./data/smplx_lite"
#     smplx_lite_segs = json.load(open(f"{smplx_lite_dir}/spec_verts_idx.json"))
...
def tensors_to_video(tensors, output_video_path, fps=25):
    """
    使用imageio将图像tensor列表转换为视频文件。

    参数:
    - tensors: 图像tensor的列表，每个tensor的形状为[1, 3, H, W]
    - output_video_path: 输出视频的路径
    - fps: 视频的帧率，默认为25
    """
    # 检查列表是否非空
    if not tensors:
        raise ValueError("图像tensor列表不能为空")
    
    with imageio.get_writer(output_video_path, fps=fps) as writer:
        for tensor in tensors:
            # 转换tensor的形状从[1, 3, H, W]到[H, W, 3]以适配imageio
            # 并确保数据类型为uint8
            img = np.array(tensor.detach().cpu().squeeze().permute(1, 2, 0)).astype(np.uint8)
            writer.append_data(img)

def images_folder_to_video(image_dir, output_video_path, fps=25):
    """
    将指定目录下的所有图片转换为视频文件。
    ...
    - fps: 视频的帧率，默认为25
    """
    # 获取目录下所有文件的路径，并排序（确保顺序正确）
    images_paths = [os.path.join(image_dir, f) for f in os.listdir(image_dir) if f.endswith(('.png', '.jpg', '.jpeg'))]
    
    # 按文件名中的数字进行排序
    images_paths.sort(key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))
    
    # 检查是否有图像文件
    if not images_paths:
        raise ValueError("指定目录下没有找到图像文件")
